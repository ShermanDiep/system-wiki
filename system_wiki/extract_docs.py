...
_DOC_SUBTYPE_RULES: list[tuple[str, tuple[str, ...]]] = [
    ("readme", ("readme", "getting-started", "overview")),
    ("adr", ("adr", "decision-record", "architecture decision record")),
    ("runbook", ("runbook", "playbook", "troubleshooting", "operations", "operational")),
    ("incident", ("incident", "postmortem", "post-mortem", "rca", "outage")),
    ("api_contract", ("openapi", "swagger", "api contract", "schema", "interface contract")),
    ("spec", ("spec", "requirements", "rfc", "proposal")),
...
def _first_meaningful_heading(text: str) -> str:
    """Return the first useful heading, falling back to the first heading seen."""
    first_heading = ""
    for _, title, _ in _extract_headings(text):
        if not first_heading:
            first_heading = title.strip()
        if title.lower().strip() not in _GENERIC_HEADINGS:
            return title.strip()
    return first_heading


def _infer_doc_subtype(path: Path, text: str) -> str:
    """Classify a document into a lightweight knowledge subtype."""
    haystack = " ".join(
        part for part in (
            path.name.lower(),
            path.stem.lower(),
            path.as_posix().lower(),
            _first_meaningful_heading(text).lower(),
            text[:4000].lower(),
        )
        if part
    )

    for subtype, patterns in _DOC_SUBTYPE_RULES:
    ...
    for link_text, target in links:
        ...
        if link_target_id not in seen_ids:
            ...
            nodes.append({
                ...
                "doc_subtype": _infer_doc_subtype(Path(resolved), ""),
            })
        edges.append({
