...
def extract_objc_signature(func_node, source) -> str:
    """Objective-C: - (NSString *)run / - (void)setValue:(id)value"""
    parts: list[str] = []
    ret_type = None
    for child in func_node.children:
        if child.type == "method_type":
            ret_type = _clean(_read_text(child, source))
        elif child.type in ("identifier", "keyword_selector"):
            parts.append(_clean(_read_text(child, source)))
        elif child.type in ("parameter", "parameter_list"):
            parts.append(_clean(_read_text(child, source)))
    selector = " ".join(part for part in parts if part)
    ret_text = f" -> {ret_type}" if ret_type else ""
    return _truncate((selector or "()") + ret_text)
...
_DISPATCH = {
    ...
    "tree_sitter_objc": extract_objc_signature,
}
