...
from system_wiki import build, cluster, score_all, label_communities, detect
from system_wiki import god_nodes, surprising_connections, suggest_questions
from system_wiki import generate, to_json, to_html, to_wiki, to_vault
...
Print node/edge/community counts. Offer to answer questions using `system-wiki query`.

---

## Living Wiki Mode

The wiki is a **persistent, compounding artifact** — it grows with every session.
...
Or use continuous watch: `system-wiki watch .`

### Rebuild — update graph

```bash
system-wiki .   # SHA256 cache skips unchanged files
```
...
**This is the compounding-artifact loop.** Every session should grow the wiki with what you learn. Use `system-wiki note` — it writes a timestamped markdown file into `wiki-out/ingested/` that the next rebuild will ingest automatically.

```bash
system-wiki note "<insight text>" [--link <node>] [--tag <tag>] [--title <heading>]
```
...
| Situation | Example |
|-----------|---------|
| You explained *why* something works a non-obvious way | `system-wiki note "GraphStore uses SHA256 because cache needs stable hash across runs" --link GraphStore --tag rationale` |
| You made an architectural decision with the user | `system-wiki note "Chose Leiden over Louvain for sparse-graph quality" --link cluster --tag decision` |
| You diagnosed a bug that revealed a hidden constraint | `system-wiki note "Cache invalidation requires version bump on schema change" --link cache --tag incident` |
| The user said "remember this" / "good point" / "write that down" | Always capture |
| You discovered a non-obvious link between modules | `system-wiki note "AuthMiddleware depends on RedisClient indirectly via SessionStore" --link AuthMiddleware --link RedisClient` |
...
- After capturing a batch of notes, rebuild: `system-wiki .`
...
```bash
cat << 'EOF' | system-wiki note --link GraphStore --tag decision --title "Storage format"
...
```bash
system-wiki query stats   # current size
system-wiki lint          # health
```
...
system-wiki /path/to/folder            # build from specific path
system-wiki --no-viz .                 # skip HTML viz (large graphs)
system-wiki query search <terms>       # keyword search
system-wiki query node <label>         # node details
system-wiki query neighbors <label>    # direct connections
system-wiki query community <id>       # list community members
system-wiki query path <A> <B>         # shortest path
system-wiki query gods                 # most connected nodes
system-wiki query stats                # summary statistics
system-wiki lint                       # graph health check
system-wiki watch .                    # auto-rebuild on changes
system-wiki add <url>                  # fetch URL as markdown
system-wiki note <text>                # file an insight for next rebuild
system-wiki --version                  # show version
system-wiki --help                     # show help
```
...
```bash
pip install system-wiki
pip install system-wiki[all]   # PDF + .docx/.xlsx + Leiden
```
