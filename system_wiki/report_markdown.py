...
def generate(
    ...
    M = build_module_graph(G)
    mstats = module_stats(M)
    hotspots = module_hotspots(M, top_n=5)
    bridges = module_bridges(M, top_n=5)

    lines += [
        "",
        "## Module Graph",
        f"- {mstats['nodes']} modules · {mstats['edges']} module edges · density {mstats['density']}",
        f"- Avg degree: in={mstats['avg_in_degree']} · out={mstats['avg_out_degree']} · components={mstats['weak_components']}",
    ]
    if hotspots:
        lines.append("- Hotspots:")
        for row in hotspots:
            lines.append(
                f"  - `{row['source_file']}` (total={row['total_degree']}, in={row['in_degree']}, out={row['out_degree']})"
            )
    if bridges:
        lines.append("- Bridges:")
        for row in bridges:
            lines.append(
                f"  - `{row['source_file']}` (betweenness={row['betweenness']})"
            )
