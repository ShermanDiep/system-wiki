...
def extract_objc_inheritance(node, source, class_nid, class_name, line, stem,
                              nodes, edges, seen_ids, str_path) -> None:
    """Objective-C: @interface Foo : Bar ..."""
    if node.type != "class_interface":
        return
    identifiers = [_read_text(child, source) for child in node.children if child.type == "identifier"]
    if len(identifiers) >= 2:
        _add_base(identifiers[1], "extends", class_nid, line, stem, nodes, edges, seen_ids, str_path)
...
_DISPATCH = {
    ...
    "tree_sitter_objc": extract_objc_inheritance,
