# CLI entry point: python -m system_wiki [path]
...
_HELP = f"""\
system-wiki v{_VERSION} — turn any folder into a queryable knowledge graph

Usage:
  system-wiki [path]                    Build graph (default: current dir)
  system-wiki query <command> [args]    Query the built graph
  system-wiki eval [suite.json]         Run retrieval/context benchmarks
  system-wiki lint                      Graph health check
  system-wiki watch [path] [interval]   Auto-rebuild on file changes
  system-wiki add <url> [--author name] Fetch URL, save as markdown
  system-wiki note <text> [opts]        File an insight into wiki-out/ingested/
...
Query commands:
  ...
  tests-for <label> docs-for <label>   impact <label>
  ...
  entrypoints      entrypoints-for <x> flow [--depth N] <x>
  context-for <task> why-related <A> <B> community <id>
  path <A> <B>
  gods             stats
...
def main() -> None:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "query":
        import importlib
        _query = importlib.import_module("system_wiki.query_graph")
        _query.query_main(sys.argv[2:])
        return

    if len(sys.argv) > 1 and sys.argv[1] == "eval":
        import importlib
        _eval = importlib.import_module("system_wiki.eval_benchmarks")
        sys.exit(_eval.eval_main(sys.argv[2:]))

    if len(sys.argv) > 1 and sys.argv[1] == "watch":
        import importlib
        _watch = importlib.import_module("system_wiki.watch_folder")
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "lint":
        ...
        if not graph_path.exists():
            print("[wiki] No graph found. Run `system-wiki .` first.")
            sys.exit(1)
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "note":
        from system_wiki import write_note
        note_args = sys.argv[2:]
        if not note_args:
            print("Usage: system-wiki note <text> [--link <label>] [--tag <name>] [--title <text>]")
            sys.exit(1)
        ...
        print("[wiki] Run `system-wiki .` to fold it into the graph.")
        return

    if len(sys.argv) > 1 and sys.argv[1] == "add":
        import importlib
        _ingest = importlib.import_module("system_wiki.ingest_url")
        if len(sys.argv) < 3:
            print("Usage: system-wiki add <url> [--author <name>]")
            sys.exit(1)
        url = sys.argv[2]
        author = sys.argv[4] if len(sys.argv) > 4 and sys.argv[3] == "--author" else None
        _ingest.ingest(url, author=author)
        print("[wiki] Run `system-wiki .` to rebuild the graph with ingested content.")
        return
    ...
    from system_wiki.analyze_graph import god_nodes, surprising_connections
    from system_wiki.analyze_questions import suggest_questions
    from system_wiki.build_graph import build
    from system_wiki.cluster_communities import cluster, score_all
    from system_wiki.cluster_label_communities import label_communities
    from system_wiki.detect_files import detect
    from system_wiki.export_html import to_html
    from system_wiki.export_json import to_json
    from system_wiki.export_vault import to_vault
    from system_wiki.export_wiki import to_wiki
    from system_wiki.extract_cross_reference import cross_reference
    from system_wiki.extract_docs import extract_docs
    from system_wiki.extract_public_api import extract
    from system_wiki.report_markdown import generate
